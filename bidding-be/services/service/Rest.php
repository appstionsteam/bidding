...
use com\appstions\bidding\entity\JsonUnserializable;
use com\appstions\bidding\exceptions\ServiceException;
use com\appstions\bidding\helper\Helper;
use com\appstions\bidding\helper\TranslateHelper;
...
class Rest{
	...
	private function createErrorResponse($errorCode = '', $message){
		...
		$response[self::BODY][self::FLAG_STATUS] = false;
		return $this->convertToJson($response);
	}
	...
	/**
	 * Valida si la respuesta es un error y envía el contenido 
	 * @param unknown $status
	 * @param unknown $errorCode
	 * @param unknown $messaga
	 */
	public function validateErrorResponse($status, $errorCode, $message){
		if($status){
